        ...
        <div className='flex gap-2 m-3 lg:m-3 justify-self-center'>
                 ...
                 <div className='flex '>
                    ...
                    <i className="fa-solid fa-plus" />
                       <p className='p-2'  onClick={() => navigate("/admin-dashboard/newstudentadmin")}>Add new student  </p>
                    </button>
          ...
          <tbody>
            {currentItems.map((item) => (
              <tr key={item.id} className="border-t">
                ...
                <td onClick={() => toggleDropdown(item.id)} className="relative px-4 py-2 text-sm text-gray-600 cursor-pointer">
  <i
    className="fa-solid fa-ellipsis-vertical"
    />

  {openDropdownId === item.id && (
    <div className="absolute z-10 right-6 bottom-[-75px]  w-40 bg-white border rounded shadow-lg">
      <ul className="text-sm text-gray-700">
        <li
          className="px-4 py-2 cursor-pointer hover:bg-gray-100"
          onClick={() => navigate(`/admin-dashboard/veiwstudentadmin/${item.id}`)}
        >
          view information
        </li>
        <li
          className="px-4 py-2 cursor-pointer hover:bg-gray-100"
          onClick={() => navigate(`/admin-dashboard/editestudentadminn/${item.id}`)}>
        edit courses
        </li>
        <li
          ...
          onClick={() => {
            if (window.confirm("Are you sure you want to delete this student?")) {
              ...
              // حذف من allItems مثلاً لو حابة تعملي حذف حقيقي
            }
          }}
        >
          Delete
